...
impl DioxusNativeWindowRenderer {
    ...
    pub async fn new_async() -> Self {
        let mut vello_renderer = InnerRenderer::new();
        vello_renderer.prewarm().await.unwrap();
        Self::with_inner_renderer(vello_renderer)
    }
    ...
    #[cfg(target_arch = "wasm32")]
    pub async fn with_features_and_limits_async(
        features: Option<Features>,
        limits: Option<Limits>,
    ) -> Self {
        let mut vello_renderer = InnerRenderer::with_options(VelloRendererOptions {
            features,
            limits,
            ..Default::default()
        });
