...
use blitz_shell::{
    create_default_event_loop, BlitzShellEvent, BlitzShellProxy, WindowConfig,
};
...
pub fn launch_cfg_with_props<P: Clone + 'static, M: 'static>(
    ...
    for mut cfg in configs {
        ...
        ))]
        {
            ...
            cfg = try_read_config!(cfg, renderer, DioxusNativeWindowRenderer);
        }
        cfg = try_read_config!(cfg, window_attributes, WindowAttributes);
        cfg = try_read_config!(cfg, config, Config);
        cfg = try_read_config!(cfg, document_config, DocumentConfig);
        let _ = cfg;
    }
    ...
    let mut document_config = document_config.unwrap_or_else(|| {
        let mut document_config = DocumentConfig::default();
        if let Some(config) = config {
            document_config.base_url = config.base_url;
            if !config.stylesheets.is_empty() {
                document_config.ua_stylesheets = Some(config.stylesheets);
            }
        }
        document_config
    });
    if document_config.net_provider.is_none() {
        document_config.net_provider = Some(net_provider);
    }
    if document_config.html_parser_provider.is_none() {
        document_config.html_parser_provider = html_parser_provider;
    }
    if document_config.navigation_provider.is_none() {
        document_config.navigation_provider = navigation_provider;
    }

    // Create document + window from the baked virtualdom
    let doc = DioxusDocument::new(vdom, document_config);
    #[cfg(any(
        feature = "vello",
        all(
            not(feature = "alt-renderer"),
    ...
    let renderer = renderer
        .unwrap_or_else(|| DioxusNativeWindowRenderer::with_features_and_limits(features, limits));
    #[cfg(not(any(
        feature = "vello",
        all(
            not(feature = "alt-renderer"),
