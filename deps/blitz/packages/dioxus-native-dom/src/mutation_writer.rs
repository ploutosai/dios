    ...
    fn create_event_listener(&mut self, name: &'static str, id: ElementId) {
        // DEBUG: log every call
        {
            ...
            if let Ok(mut f) = std::fs::OpenOptions::new()
                .create(true)
                .append(true)
                .open("/tmp/editor_debug.log")
            {
                let _ = writeln!(
                    f,
                    "[MUTATION] create_event_listener name={:?} id={}",
                    name, id.0
                );
            }
        }
        ...
        let parse_result = DomEventKind::from_str(name);
        {
            use std::io::Write;
            if let Ok(mut f) = std::fs::OpenOptions::new()
                .create(true)
                ...
                .open("/tmp/editor_debug.log")
            {
                let _ = writeln!(
                    ...
                    "[MUTATION] from_str({:?}) = {:?}",
                    name,
                    parse_result.as_ref().map(|k| k.discriminant())
                );
            }
        }
        if let Ok(kind) = parse_result {
            ...
            self.state.event_handler_counts[idx] += 1;
            {
                    ...
                    .open("/tmp/editor_debug.log")
                {
                    let _ = writeln!(
                        f,
                        "[MUTATION] counts ptr={:p} counts[{}]={}",
                        &self.state.event_handler_counts, idx, self.state.event_handler_counts[idx]
                    );
                }
            }
        }
