...
impl NodeHandle {
    ...
    /// Get the client bounding rect synchronously. Safe to call from event handlers.
    ///
    /// Resolves layout before reading. `onmounted` fires immediately after
    /// `vdom.rebuild()`, before the first paint, so node sizes would otherwise
    /// be zero. Resolve is idempotent when the tree is already clean.
    pub fn client_rect_sync(&self) -> Option<PixelsRect> {
        {
            let mut doc = self.doc.borrow_mut();
            doc.resolve(0.0);
        }
        let doc = self.doc.borrow();
        doc.get_client_bounding_rect(self.node_id).map(|r| {
            PixelsRect::new(
                Point2D::new(r.x, r.y),
...
impl RenderedElementBacking for NodeHandle {
    ...
    fn get_scroll_offset(&self) -> Pin<Box<dyn Future<Output = MountedResult<PixelsVector2D>>>> {
        ...
        Box::pin(async move {
            let doc = doc.borrow();
            let node = doc.get_node(node_id).ok_or(MountedError::OperationFailed(
                Box::from("Node does not exist"),
            ))?;
            Ok(PixelsVector2D::new(node.scroll_offset.x, node.scroll_offset.y))
        })
    }
    ...
    fn get_scroll_size(&self) -> Pin<Box<dyn Future<Output = MountedResult<PixelsSize>>>> {
        ...
        Box::pin(async move {
            ...
            Ok(PixelsSize::new(scroll_width, scroll_height))
        })
    }

    fn get_client_rect(&self) -> Pin<Box<dyn Future<Output = MountedResult<PixelsRect>>>> {
        // Compute synchronously in the Future constructor (not in the async body)
        // so this is safe to call from event handlers and even from spawn contexts
        // (no doc borrow deferred into the polled async block).
        let result = {
            let doc = self.doc.borrow();
            match doc.get_client_bounding_rect(self.node_id) {
                ...
                None => Err(MountedError::OperationFailed(
                    Box::from("Node does not exist"),
                )),
            }
        };
    ...
    fn scroll_to(
        ...
        // TODO: implement scroll-into-view logic
    ...
    fn scroll(
        ...
        use blitz_dom::util::Point;
        let Ok(mut doc) = self.doc.try_borrow_mut() else {
            // Drag autoscroll can race with event dispatch borrowing the document.
            // Dropping one scroll tick is harmless; panicking is not.
            return Box::pin(async { Ok(()) });
        };
        if let Some(node) = doc.get_node_mut(self.node_id) {
            let max_x = node.final_layout.scroll_width() as f64;
            let max_y = node.final_layout.scroll_height() as f64;
            node.scroll_offset = Point {
                x: coordinates.x.clamp(0.0, max_x),
                y: coordinates.y.clamp(0.0, max_y),
            };
        }
