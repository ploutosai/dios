...
impl BaseDocument {
    ...
    pub fn resolve(&mut self, current_time_for_animations: f64) {
        ...
        {
            puffin::profile_scope!("resolve: handle_messages");
            self.handle_messages();
        }

        {
            puffin::profile_scope!("resolve: scroll_animation");
        ...
        {
            puffin::profile_scope!("resolve: stylist (style recalc)");
        ...
        #[cfg(feature = "incremental")]
        {
            puffin::profile_scope!("resolve: propagate_damage_flags");
        ...
        {
            puffin::profile_scope!("resolve: layout_children (box construction)");
            self.resolve_layout_children();
        }
        timer.record_time("construct");

        {
            puffin::profile_scope!("resolve: deferred_tasks (inline layout / parley)");
        ...
        {
            puffin::profile_scope!("resolve: flush_styles_to_layout");
        ...
        {
            puffin::profile_scope!("resolve: layout (taffy)");
            self.resolve_layout();
        }
        ...
        #[cfg(feature = "incremental")]
        {
            puffin::profile_scope!("resolve: clear_damage");
        ...
        {
            puffin::profile_scope!("resolve: subdocs");
    ...
    pub fn resolve_deferred_tasks(&mut self) {
        ...
        puffin::profile_scope!(
            "deferred tasks",
            format!("{} tasks", deferred_construction_nodes.len())
        );
        ...
        let results: Vec<ConstructionTaskResult> = iter
            .map(|task: ConstructionTask| match task.data {
                ConstructionTaskData::InlineLayout(mut layout) => {
                    puffin::profile_scope!("build_inline_layout (parley)");
                    #[cfg(feature = "parallel-construct")]
