...
impl BaseDocument {
    ...
    pub fn get_text_selection_ranges(&self) -> Vec<(usize, usize, usize)> {
        // Native document text selection is disabled for this shell/editor.
        // Returning no ranges is the final guard that prevents any stale or
        // accidentally-created selection from ever being painted (the blue
        // system selection). The editor renders its own selection rectangles.
        return Vec::new();
        ...
        let anchor_node = match self.text_selection.anchor.resolve_node_id(lookup) {
            Some(id) if self.nodes.contains(id) => id,
            _ => return Vec::new(),
        };
