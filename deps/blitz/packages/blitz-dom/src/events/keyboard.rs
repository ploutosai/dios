...
fn apply_keypress_event(
    ...
    match event.key {
        ...
        // Plain printable characters are intentionally NOT inserted here.
        // Shells like blitz-shell-miniquad fire both a KeyDown (with
        // Key::Character) and a separate IME Commit event for the same
        // keystroke; if we inserted on both, every typed character would
        // appear twice in the input. IME Commit (handled in events/ime.rs)
        // is the canonical source of typed text. KeyDown still handles
        // navigation/control keys above (arrows, Enter, Backspace, etc.).
        Key::Character(_) => {}
        _ => {}
    };
