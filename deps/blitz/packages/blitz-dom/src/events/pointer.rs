...
pub(crate) fn handle_pointermove<F: FnMut(DomEvent)>(
    ...
    // Check if we've moved enough to be considered a selection drag. Keep this
    // threshold in sync with double-click detection; otherwise tiny hand jitter
    // suppresses click/dblclick and makes app-level `ondoubleclick` unreliable.
        ...
        if dx.abs() > CLICK_SLOP_PX || dy.abs() > CLICK_SLOP_PX {
            match event.id {
                BlitzPointerId::Mouse | BlitzPointerId::Pen => {
                    // Native document text selection is disabled for this shell.
                    // Apps that need selection (like the editor) implement their
                    // own selection layer from pointer events.
                }
...
pub(crate) fn handle_pointerdown(
    ...
    let elapsed_ok = doc
        ...
        .unwrap_or(false);
    let dx = (doc.mousedown_position.x - x).abs();
    let dy = (doc.mousedown_position.y - y).abs();
    let position_ok = dx <= CLICK_SLOP_PX && dy <= CLICK_SLOP_PX;
    doc.click_count = if elapsed_ok && position_ok {
        doc.click_count + 1
    } else {
        1
    };
    ...
    match click_target {
        ...
        ClickTarget::SelectableText => {
            // Native document text selection is disabled. Keep clearing any
            // stale selection, but never create/extend one here.
            doc.clear_text_selection();
        }
        ClickTarget::TextInput { content_box_offset } => {
    ...
    // Resolve focus on pointerdown (browser behavior): walk up from the hit
    // node to find the nearest focusable ancestor and focus it. If none is
    // found, clear focus.
    let mut maybe_id = Some(hit.node_id);
    let mut found_focusable = false;
    while let Some(nid) = maybe_id {
        if doc.nodes[nid].is_focussable() {
...
pub(crate) fn handle_click(
    ...
    // Focus is handled in handle_pointerdown, not here.
...
pub(crate) fn handle_wheel<F: FnMut(DomEvent)>(
    ...
    let anchor = doc
        .get_hover_node_id()
        .filter(|&node_id| doc.nodes.contains(node_id));
    let has_changed = doc.scroll_by(anchor, scroll_x, scroll_y, &mut dispatch_event);
    if has_changed {
        doc.shell_provider.request_redraw();
    }
}
