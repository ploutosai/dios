...
impl<'doc, Handler: EventHandler> EventDriver<'doc, Handler> {
    ...
    pub fn handle_ui_event(&mut self, event: UiEvent) {
        ...
        match &event {
            ...
            UiEvent::Wheel(event) => {
                // Wheel events can arrive without an intervening pointer-move.
                // After scrolling/virtualization, the old hover node may be
                // stale or no longer under the cursor, which makes subsequent
                // wheel events target the wrong node (or root) and appear to be
                // ignored. Refresh hover from the wheel coordinates every time.
                let mut doc = self.doc.inner_mut();
    ...
    fn handle_pointer_event(
        ...
        let is_pointer_down = matches!(ptr_event.data, DomEventData::PointerDown(_));
        let mut event_state = EventState::default();
        event_state = self.run_handler_event(&mut ptr_event, event_state);

        // PointerDown default action updates internal click-count/focus state.
        // Run it before the compatibility MouseDown event so an app-level
        // `onmousedown: prevent_default()` doesn't suppress double-click
        // bookkeeping. This matches browser behavior more closely: preventing
        // the mouse event's default action should not make `dblclick` stop
        // existing.
        if !event_state.is_cancelled() && is_pointer_down {
