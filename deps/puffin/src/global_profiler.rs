...
impl GlobalProfiler {
    ...
    /// Open a new frame for scope accumulation.
    ///
    /// Discards anything reported since the previous [`end_frame`] —
    /// callers use this to declare "the next frame really starts here,
    /// drop the leftovers." In practice the leftovers come from
    /// background threads (e.g. `puffin_http`'s send loop firing its
    /// own `profile_function!` between our main-thread frames); without
    /// this clear they would surface to the viewer as standalone
    /// near-empty frames.
    ...
    pub fn end_frame(&mut self) {
        ...
        let new_frame = match FrameData::new(
            ...
            Err(Error::Empty) => {
                // Drop empty frames without consuming an index — the
                // viewer never sees a gap, and "no real work happened"
                // doesn't materialise as a phantom frame.
                return;
            }
            Err(err) => {
    ...
    ///
    /// Always accumulates into [`current_frame`](Self::current_frame).
    /// Scopes reported between an [`end_frame`](Self::end_frame) and the
    /// next [`begin_frame`](Self::begin_frame) are silently discarded
    /// by the next `begin_frame`, which clears the buffer.
    pub fn report(
        &mut self,
        info: ThreadInfo,
