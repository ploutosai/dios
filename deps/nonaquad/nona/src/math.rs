...
impl Rect {
    ...
    pub fn intersect(self, rect: Rect) -> Rect {
        // Previously both `a` and `b` were destructured from `rect`, so this
        // returned the second rect unchanged regardless of `self`. The bug
        // propagated through `Context::intersect_scissor`, breaking nested
        // overflow clips (a child's clip rect would replace its ancestor's
