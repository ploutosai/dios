...
impl Context {
    ...
    /// Render pre-positioned glyphs specified by glyph ID.
    /// Used by external layout engines that provide their own glyph IDs and positions.
    /// The current font (set via `fontid()`) and font_size are used.
    pub fn draw_glyphs_by_id<R: Renderer>(
        &mut self,
        renderer: &mut R,
        glyphs: &[crate::fonts::GlyphPosition],
    ) -> Result<(), NonaError> {
        use crate::fonts::LayoutGlyph;

        let state = self.states.last().unwrap();
        let scale = state.xform.font_scale() * self.device_pixel_ratio;
        let invscale = 1.0 / scale;
        let font_size = state.font_size * scale;
        let font_id = state.font_id;

        // Scale glyph positions
        let scaled_glyphs: Vec<crate::fonts::GlyphPosition> = glyphs
            .iter()
            .map(|g| crate::fonts::GlyphPosition {
                glyph_id: g.glyph_id,
                x: g.x * scale,
                y: g.y * scale,
            })
        ...
        let mut layout_result: Vec<LayoutGlyph> = Vec::new();
        self.fonts.layout_glyphs_by_id(
            renderer,
            font_id,
            font_size,
            &scaled_glyphs,
        ...
        for lg in &layout_result {
            // Compute local-space positions, then transform to screen space
            // since triangles() renders directly without applying the transform.
            let lt_local = Point::new(lg.bounds.min.x * invscale, lg.bounds.min.y * invscale);
            let rt_local = Point::new(lg.bounds.max.x * invscale, lg.bounds.min.y * invscale);
            let lb_local = Point::new(lg.bounds.min.x * invscale, lg.bounds.max.y * invscale);
            let rb_local = Point::new(lg.bounds.max.x * invscale, lg.bounds.max.y * invscale);

            let lt = xform.transform_point(lt_local);
            let rt = xform.transform_point(rt_local);
            let lb = xform.transform_point(lb_local);
            let rb = xform.transform_point(rb_local);

            self.cache
                .vertexes
                .push(Vertex::new(lt.x, lt.y, lg.uv.min.x, lg.uv.min.y));
            self.cache
                .vertexes
                .push(Vertex::new(rb.x, rb.y, lg.uv.max.x, lg.uv.max.y));
            self.cache
                .vertexes
                .push(Vertex::new(rt.x, rt.y, lg.uv.max.x, lg.uv.min.y));
            ...
            self.cache
                .vertexes
                .push(Vertex::new(lb.x, lb.y, lg.uv.min.x, lg.uv.max.y));
            self.cache
                .vertexes
                .push(Vertex::new(rb.x, rb.y, lg.uv.max.x, lg.uv.max.y));
        }
    ...
    /// Get the advance width for a glyph by ID, using the current font and font_size.
    pub fn glyph_advance_width(&self, glyph_id: u16) -> Option<f32> {
        let state = self.states.last().unwrap();
        self.fonts
            .advance_width(state.font_id, glyph_id, state.font_size)
    }
