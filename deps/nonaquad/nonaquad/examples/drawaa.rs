...
impl Stage {
    pub fn new() -> Stage {
        ...
        let mut renderer = nvgimpl::Renderer::create(&mut *ctx).unwrap();
        let mut nona = nona::Context::create(&mut renderer.with_context(&mut *ctx)).unwrap();
