...
impl WaylandPayload {
    ...
    unsafe fn poll_new_event(&mut self, blocking: bool) {
        ...
        ) > 0
        {
            ...
            if fds[2].revents & libc::POLLIN == libc::POLLIN {
                crate::native::drain_event_loop_waker(self.wake_fd);
            }
        } else {
            (self.client.wl_display_cancel_read)(self.display);
        }
    ...
    F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
    unsafe {
        ...
        let (wake_fd, wake_event_loop) = crate::native::make_event_loop_waker().ok()?;
