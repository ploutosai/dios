    ...
    F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
    ...
    let (wake_fd, wake_event_loop) = crate::native::make_event_loop_waker().unwrap();
    ...
    crate::set_display(NativeDisplayData {
        ...
        wake_event_loop: Some(wake_event_loop),
    ...
    while !crate::native_display().try_lock().unwrap().quit_ordered {
        ...
        if block_on_wait && (display.libx11.XPending)(display.display) == 0 {
            let x11_fd = (display.libx11.XConnectionNumber)(display.display);
            let mut fds = [
                libc::pollfd {
                    fd: x11_fd,
                    events: libc::POLLIN,
            ...
            {
                crate::native::drain_event_loop_waker(wake_fd);
            }
            while let Ok(request) = rx.try_recv() {
                display.process_request(request);
            }
